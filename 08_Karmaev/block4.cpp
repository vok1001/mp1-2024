...
public:
	...
	void printMon() const {
		cout << "ƒата: " << day << "." << month << "." << year << "  " << hour << ":00  " << temperature << " C" << endl;
	}
...
public:
	...
	void getTemper(int d, int m, int y, int h)const {
		...
		for (int i = 0; i < monitor.size(); i++) {
			if (d == monitor[i].getDay() && m == monitor[i].getMonth() && y == monitor[i].getYear() && h == monitor[i].getHour()) {
				cout << "“емпература -   " << monitor[i].getTemperature() << " C" << endl;
				k = 1;
			}
		}
		if (k != 1) {
			cout << "“акого замера не было (((";
		}
	}
	...
	void addMonitoringSeries(int d, int m, int y, int k) {
		...
		while (k != 0) {
			...
			while (h < 0 || h > 24) {
				...
				cout << "ќшибка введите заново" << endl;
				cin >> h;
	...
	void findAvgTempForDay(int d, int m, int y) {
		...
		cout << "—редн€€ температура за " << d<<"."<<m<<"."<<y<<"  "<<(c_temp / counter);
	}
	...
	void findAvgTempForMonth(int m, int y) {
		...
		cout << "—редн€€ температура за мес€ц = " << (c_temp / counter);
	}
	...
	void findAvgTempForYear(int m, int y) {
		...
		cout << "—редн€€ температура за год = " << (c_temp / counter);
	}
	...
	void avgdaytempformonth(int m, int y) {
		...
		cout << "—редн€€ дневна€ температура за мес€ц = " << (c_temp / counter);
	}
	...
	void avgnighttempformonth(int m, int y) {
		...
		cout << "—редн€€ ночна€ температура за мес€ц = " << (c_temp / counter);
	}
